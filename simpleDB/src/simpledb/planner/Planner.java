...
//   }
   ...
   public Plan createQueryPlan(String qry, Transaction tx) {
	   Parser parser = new Parser(qry);
	   QueryData data = parser.query(tx); //Bir tablonun schema bilgisini elde etmek icin tx parametresi gonderiliyor.
	   return qplanner.createPlan(data, tx);
   }
...
//   }
   ...
   public int executeUpdate(String cmd, Transaction tx) {
	   Parser parser = new Parser(cmd);
	   Object obj = parser.updateCmd(tx); //Bir tablonun schema bilgisini elde etmek icin tx parametresi gonderiliyor.
	   if (obj instanceof InsertData)
		   return uplanner.executeInsert((InsertData)obj, tx);
	   else if (obj instanceof DeleteData)
		   return uplanner.executeDelete((DeleteData)obj, tx);
