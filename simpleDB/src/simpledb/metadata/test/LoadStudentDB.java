...
//      sectmax=25000, enrollmax=1500000;
	...
	//MEDIUM DB:
	public static int coursemax=32, studentmax=450, deptmax=4,
		      sectmax=250, enrollmax=1500;  // //V(prof)=25  Vgrade=14  Vyearoffered=50
	
	//SMALL DB:
//	public static int coursemax=10, studentmax=45, deptmax=2,
//		      sectmax=25, enrollmax=500;
...
//   public static void main(String[] args) {
      ...
      if (SimpleDB.fileMgr().isNew()) {
         ...
         for (int id=0; id<studentmax; id++) {
            ...
            System.out.println(id + " "+ "student"+id+" "+ ((id%50)+1960)+ " "+id%deptmax);
         }
         rf.close();
         ...
         for (int id=0; id<coursemax; id++) {
            ...
            System.out.println(id + " "+ "course"+id+" "+ (id%deptmax));
         }
         rf.close();
         ...
         for (int id=0; id<sectmax; id++) {
            ...
            int profnum = id%25;   //V(prof)=25
            rf.setString("prof", "prof"+profnum);            
            rf.setInt("yearoffered", (id%50)+1960);
            System.out.println(id + " "+ (id%coursemax)+  " " + "prof"+profnum+" "+ ((id%50)+1960));
         }
         rf.close();
         ...
         rf.beforeFirst();
         System.out.println("ENROLL TABLE");
         System.out.println(" ------------------------ ");
         for (int id=0; id<enrollmax; id++) {
            ...
            System.out.println(id + " "+ (id%studentmax)+ " "+(id%sectmax)+  " " + aGrade);
         }
         rf.close();
         tx.commit();
         tx = new Transaction();
...
public static void setDataStatistics(Transaction tx) {
   ...
   public static  void getDataStatistics(Transaction tx){
	   ...
	   System.out.println("STUDENT: [B=" +si.blocksAccessed()+ ", R="+ si.recordsOutput() + "] V(sid,sname,gradyear,majorid)="
		...
		System.out.println("DEPT: [B=" +si.blocksAccessed()+ ", R="+ si.recordsOutput() + "] V(did,dname)="
				+ si.distinctValues("did")+ " "+ si.distinctValues("dname"));
		
		ti = md.getTableInfo("course", tx);
		si=md.getStatInfo("course", ti, tx);
		System.out.println("COURSE: [B=" +si.blocksAccessed()+ ", R="+ si.recordsOutput() + "] V(cid,title,deptid)="
				   						+ si.distinctValues("cid")+ " "+ si.distinctValues("title")+ " "
				   						+ si.distinctValues("deptid")+ " ");
		
		ti = md.getTableInfo("section", tx);
		si=md.getStatInfo("section", ti, tx);
		System.out.println("SECTION: [B=" +si.blocksAccessed()+ ", R="+ si.recordsOutput() + "] V(sectid,courseid,prof,yearoffered)="
		...
		System.out.println("ENROLL: [B=" +si.blocksAccessed()+ ", R="+ si.recordsOutput() + "] V(eid,sectionid,studentid,grade)="
