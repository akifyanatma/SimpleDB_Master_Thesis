...
//		testDrIndexAll();  // This function ONLY removes idx entriesin the catalog. MUST Remove from file system manually.If not, next creat index will RE-load and results with an invalid index.
		
		//below 3 functions does not use any index.
//		 testInsert();
//		 testDelete();
//		 testModify();
...
//		testJoinQueryWithIndex();
	...
	private static void testCrDrIndex(){
		...
		if(success>0)
			System.out.println(" an idx is available.");
		else
			System.out.println(" idx already exists.");
		
		// Ayný tx'da drop yaparsak, tx.commit olaiyor, cunku silinen idx dosyasýna ait olan
		// bloklarý flush yapmak istedigi zaman dosya silindigi icin yapamayacak.
		// Bunun icin "simdilik" ancak yeni bir tx ile idx'i hem catalog hem de file system'den
		// silebilriiz. Bu yuzden asagisiný COMMENT ile kapatýyoruz.
...
//		if(success>0)
//			System.out.println(success + " idx is deleted from catalog. Drop file with another tx. Or, Delete manually if necessary.");
//		
		tx.commit();
	}
	private static void testDrIndex(){
		...
		if(success>0)
			System.out.println(success + " idx is deleted from catalog and File system. If not, next \"creat index\" will RE-load over invalid index.");
		else 
			System.out.println(" idx does NOT already exist.");
	...
	private static void testDrIndexAll(){  
		...
		didx="drop indexall on enroll"; // This prints "1 idx are deleted....". Because we drop indexes in the ENROLL. there are 1 idx over it.
		planner=SimpleDB.planner(PlannerType.BASIC);
		success=planner.executeUpdate(didx, tx);
		if(success>0)
			System.out.println(success + " idxs are deleted from catalog. CHECK File Mgr. MUST Delete ALL idxs manually");
		
		didx="drop indexall on dept"; // Nothing prints. We have no idx on dept.
		planner=SimpleDB.planner(PlannerType.BASIC);
		success=planner.executeUpdate(didx, tx);
		if(success>0)
			System.out.println(success + " idxs are deleted from catalog. CHECK File Mgr. MUST Delete ALL idxs manually");
	...
	private static void testInsertWithIndex() {
		...
		Planner planner=SimpleDB.planner(PlannerType.HEURISTIC6a); // BASIC de yapsak bulk load da farketmez. Ancak ilerideki tek tek insert iþlemlernde INDEXUPDATE kullanmak için simdiden HEURISTIC yaptýk.
		int success=planner.executeUpdate(cidx, tx);
...
private static void testJoinQueryWithIndex() {
		...
		Transaction tx = new Transaction();		
		String cidx="create index majoridbtreeidx on student(majorid)";
		Planner planner=SimpleDB.planner(PlannerType.HEURISTIC6a);
		int success=planner.executeUpdate(cidx, tx);
